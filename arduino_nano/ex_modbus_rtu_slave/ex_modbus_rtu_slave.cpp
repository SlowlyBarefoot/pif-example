...
void setup()
{
    ...
    if (!pifUart_AttachTxTask(&g_uart_modbus, PIF_ID_AUTO, TM_EXTERNAL, 0, NULL)) return;
    if (!pifUart_AttachRxTask(&g_uart_modbus, PIF_ID_AUTO, TM_PERIOD, 5000, NULL)) return;	// 5ms
	...
	if (!pifModbusRtuSlave_Init(&g_modbus_slave, PIF_ID_AUTO, &g_timer_1ms, RTU_SERVER_ADDRESS)) return;
