...
void setup()
{
    ...
    s_pstTimer1msRed = pifPulse_AddItem(s_pstTimer1ms, PT_enOnce);
    if (!s_pstTimer1msRed) return;
    pifPulse_AttachEvtFinish(s_pstTimer1msRed, led_red_toggle, NULL);
    pifPulse_StartItem(s_pstTimer1msRed, 200);						// 200ms

    s_pstTimer1msYellow = pifPulse_AddItem(s_pstTimer1ms, PT_enOnce);
    if (!s_pstTimer1msYellow) return;
    pifPulse_AttachEvtFinish(s_pstTimer1msYellow, led_yellow_toggle, NULL);

    s_pstTimer1msGreen = pifPulse_AddItem(s_pstTimer1ms, PT_enOnce);
    if (!s_pstTimer1msGreen) return;
    pifPulse_AttachEvtFinish(s_pstTimer1msGreen, led_green_toggle, NULL);
}
