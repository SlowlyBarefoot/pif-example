...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	if (huart->Instance == USART2) {
		state = pifComm_EndGetTxData(&g_comm, s_log_tx);
		if (state & PIF_COMM_SEND_DATA_STATE_EMPTY) {
			pifComm_FinishTransfer(&g_comm);
		}
		else {
			s_log_tx = 0;
			state = pifComm_StartGetTxData(&g_comm, &p_data, &s_log_tx);
			if (state & PIF_COMM_SEND_DATA_STATE_DATA) {
...
{
	if (huart->Instance == USART2) {
		pifComm_PutRxByte(&g_comm, s_log_rx);
