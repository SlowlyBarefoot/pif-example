...
int main(void)
{
  ...
  pif_Init(micros);
    ...
    if (!pifUart_AttachTxTask(&g_uart_host, TM_PERIOD, 50000, "UartTxHost")) { line = __LINE__; goto fail; }		// 50ms
    if (!pifUart_AttachRxTask(&g_uart_host, TM_PERIOD, 1000, "UartRxHost")) { line = __LINE__; goto fail; }			// 1ms
