...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	if (huart->Instance == USART2) {
		...
		state = pifUart_EndGetTxData(&g_uart_host, s_host_tx);
		if (!(state & PIF_UART_SEND_DATA_STATE_EMPTY)) {
			s_host_tx = 0;
			state = pifUart_StartGetTxData(&g_uart_host, &p_data, &s_host_tx);
			if (state & PIF_UART_SEND_DATA_STATE_DATA) {
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	if (huart->Instance == USART2) {
		...
		pifUart_PutRxByte(&g_uart_host, s_host_rx);
...
int main(void)
{
  ...
  pif_Init(micros);
	...
	if (!pifUart_Init(&g_uart_host, PIF_ID_AUTO, huart6.Init.BaudRate)) { line = __LINE__; goto fail; }
    if (!pifUart_AttachTxTask(&g_uart_host, PIF_ID_AUTO, TM_PERIOD, 50000, "UartTxHost")) { line = __LINE__; goto fail; }	// 50ms
    if (!pifUart_AttachRxTask(&g_uart_host, PIF_ID_AUTO, TM_PERIOD, 1000, "UartRxHost")) { line = __LINE__; goto fail; }	// 1ms
    if (!pifUart_AllocRxBuffer(&g_uart_host, 32)) { line = __LINE__; goto fail; }											// 32bytes
    if (!pifUart_AllocTxBuffer(&g_uart_host, 128)) { line = __LINE__; goto fail; }											// 128bytes
    g_uart_host.act_start_transfer = actHostStartTransfer;
