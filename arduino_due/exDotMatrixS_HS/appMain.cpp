...
static uint32_t _taskDotMatrixTest(PifTask* p_task)
{
	...
	switch (shift) {
	case 2:
		pifDotMatrix_ShiftOn(&g_dot_matrix, DMSD_LEFT, DMSM_ONCE, 200, 3);
		break;

	case 4:
		pifDotMatrix_ShiftOn(&g_dot_matrix, DMSD_RIGHT, DMSM_ONCE, 200, 3);
		break;

	case 10:
		pifDotMatrix_ShiftOn(&g_dot_matrix, DMSD_LEFT, DMSM_REPEAT_HOR, 200, 0);
		break;

	case 20:
		pifDotMatrix_ShiftOn(&g_dot_matrix, DMSD_RIGHT, DMSM_REPEAT_HOR, 200, 0);
		break;

	case 30:
		pifDotMatrix_ShiftOn(&g_dot_matrix, DMSD_LEFT, DMSM_PING_PONG_HOR, 200, 0);
		break;
...
BOOL appSetup()
{
    ...
    if (!pifDotMatrix_SetPatternSize(&g_dot_matrix, 1)) return FALSE;
   	if (!pifDotMatrix_AddPattern(&g_dot_matrix, 5 * 8, 8, ucPattern)) return FALSE;
