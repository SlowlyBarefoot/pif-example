...
static void _fnProtocolAnswer20(PIF_stProtocolPacket *pstPacket)
{
	...
	if (!pifProtocol_MakeAnswer(s_pstProtocol, pstPacket, stProtocolQuestions1[0].enFlags, NULL, 0)) {
		pifLog_Printf(LT_enInfo, "Answer20: PID=%d Error=%d", pstPacket->ucPacketId, pif_enError);
	}
}
...
BOOL exSerial1_Setup()
{
    ...
    for (i = 0; i < SWITCH_COUNT; i++) {
	    ...
	    if (!pifTask_Add(TM_enPeriodMs, 10, pifSensorSwitch_Task, s_stProtocolTest[i].pstPushSwitch, TRUE)) return FALSE;	// 10ms
    ...
    if (!pifTask_Add(TM_enPeriodMs, 1, pifComm_Task, g_pstSerial1, TRUE)) return FALSE;									// 1ms
