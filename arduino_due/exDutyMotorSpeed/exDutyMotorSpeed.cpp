	...
	if (argc == 1) {
		...
		if (!strcmp(argv[1], "stage")) {
			...
			if (!value) {
				...
				pifDutyMotorSpeed_Stop(s_pstMotor);
				return PIF_TERM_CMD_NO_ERROR;
			}
			else if (value <= DUTY_MOTOR_STAGE_COUNT) {
				if (!s_stDutyMotorTest.ucStage) {
					if (pifDutyMotorSpeed_Start(s_pstMotor, value - 1, 2000)) {
		...
		if (!strcmp(argv[1], "off")) {
			...
			s_stDutyMotorTest.ucStage = 0;
			pifDutyMotorSpeed_Emergency(s_pstMotor);
			return PIF_TERM_CMD_NO_ERROR;
		}
...
{
	PIF_stDutyMotorSpeed *pstInfo = (PIF_stDutyMotorSpeed *)pvInfo;
...
static void _taskInitPos(PIF_stTask *pstTask)
{
	...
	case 2:
		if (!s_stDutyMotorTest.ucStage) {
			if (s_pstSwitch[0]->_swCurrState == ON) {
				...
				if (pifDutyMotorSpeed_Start(s_pstMotor, 0, unTime)) {
	...
	case 3:
		if (!s_stDutyMotorTest.ucStage) {
			if (pifDutyMotorSpeed_Start(s_pstMotor, 1, unTime)) {
...
void setup()
{
    ...
    pifDutyMotorSpeed_AddStages(s_pstMotor, DUTY_MOTOR_STAGE_COUNT, s_stDutyMotorStages);
