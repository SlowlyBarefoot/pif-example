...
PifModbusBitField server_discrete_inputs[COILS_ADDR_MAX] = {0};
PifModbusBitField server_coils[COILS_ADDR_MAX] = {0};
...
BOOL appSetup()
{
    ...
    for (i = 0; i < COILS_ADDR_MAX; i++) {
    	server_discrete_inputs[i] = i;
    	server_coils[i] = i;
    }
    for (i = 0; i < REGS_ADDR_MAX; i++) {
    	server_holding_registers[i] = i;
    	server_input_registers[i] = i;
    }
