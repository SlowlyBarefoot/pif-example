...
PifLed g_led_l;
PifModbusAsciiMaster g_modbus_master;
...
static uint32_t _taskResult(PifTask *p_task)
{
	...
	switch (pifModbusAsciiMaster_Check(&g_modbus_master)) {
	case MBMR_DONE:
...
static int _CmdReadCoils(int argc, char *argv[])
{
	...
	if (argc == 0) {
		...
		if (qty > 0 && addr + qty < COILS_ADDR_MAX * 8) {
			...
			if (!pifModbusAsciiMaster_ReadCoils(&g_modbus_master, slave, addr, qty, coils)) return PIF_LOG_CMD_INVALID_ARG;
...
static int _CmdReadDiscreteInputs(int argc, char *argv[])
{
	...
	if (argc == 0) {
		...
		if (qty > 0 && addr + qty < COILS_ADDR_MAX * 8) {
			PIF_MODBUS_RESET_BIT_FIELD(coils);
			if (!pifModbusAsciiMaster_ReadDiscreteInputs(&g_modbus_master, slave, addr, qty, coils)) return PIF_LOG_CMD_INVALID_ARG;
...
static int _CmdReadHoldingRegs(int argc, char *argv[])
{
	...
	if (argc == 0) {
		...
		if (qty > 0 && addr + qty < REGS_ADDR_MAX) {
			if (!pifModbusAsciiMaster_ReadHoldingRegisters(&g_modbus_master, slave, addr, qty, r_regs)) return PIF_LOG_CMD_INVALID_ARG;
...
static int _CmdReadInputRegs(int argc, char *argv[])
{
	...
	if (argc == 0) {
		...
		qty = atoi(argv[2]);
		if (qty > 0 && addr + qty < REGS_ADDR_MAX) {
			if (!pifModbusAsciiMaster_ReadInputRegisters(&g_modbus_master, slave, addr, qty, r_regs)) return PIF_LOG_CMD_INVALID_ARG;
			_expectResult("Regs", MBRK_REGISTERS, addr, qty, slave);
		}
		else {
			return PIF_LOG_CMD_INVALID_ARG;
...
static int _CmdWriteSingleCoil(int argc, char *argv[])
{
	...
	if (argc == 0) {
		...
		if (!pifModbusAsciiMaster_WriteSingleCoil(&g_modbus_master, slave, addr, value)) return PIF_LOG_CMD_INVALID_ARG;
...
static int _CmdWriteSingleReg(int argc, char *argv[])
{
	...
	if (argc == 0) {
		...
		if (!pifModbusAsciiMaster_WriteSingleRegister(&g_modbus_master, slave, addr, value)) return PIF_LOG_CMD_INVALID_ARG;
		_expectResult(NULL, MBRK_NONE, addr, 1, slave);
		return PIF_LOG_CMD_NO_ERROR;
	}
	return PIF_LOG_CMD_TOO_FEW_ARGS;
...
static int _CmdWriteMultipleCoils(int argc, char *argv[])
{
	...
	if (argc == 0) {
		...
		if (qty > 0 && addr + qty < COILS_ADDR_MAX * 8) {
			...
			if (!pifModbusAsciiMaster_WriteMultipleCoils(&g_modbus_master, slave, addr, qty, coils)) return PIF_LOG_CMD_INVALID_ARG;
...
static int _CmdWriteMultipleRegs(int argc, char *argv[])
{
	...
	if (argc == 0) {
		...
		if (qty > 0 && addr + qty < REGS_ADDR_MAX) {
			...
			if (!pifModbusAsciiMaster_WriteMultipleRegisters(&g_modbus_master, slave, addr, qty, w_regs)) return PIF_LOG_CMD_INVALID_ARG;
			_expectResult(NULL, MBRK_NONE, addr, qty, slave);
		}
		else {
			return PIF_LOG_CMD_INVALID_ARG;
...
static int _CmdReadWriteMultipleRegs(int argc, char *argv[])
{
	...
	if (argc == 0) {
		...
		if (qty > 0 && r_addr + qty < REGS_ADDR_MAX && w_addr + qty < REGS_ADDR_MAX) {
			...
			if (!pifModbusAsciiMaster_ReadWriteMultipleRegisters(&g_modbus_master, slave, r_addr, qty, r_regs, w_addr, qty, w_regs)) return PIF_LOG_CMD_INVALID_ARG;
