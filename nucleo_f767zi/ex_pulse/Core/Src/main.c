...
{
	pifPulse_sigEdge(&g_pulse, HAL_GPIO_ReadPin(GPIOA, GPIO_Pin) ? PS_RISING_EDGE : PS_FALLING_EDGE, micros());
}
...
int main(void)
{
  ...
  if (!pifUart_AttachTxTask(&g_uart_log, PIF_ID_AUTO, TM_EXTERNAL, 0, NULL)) return -1;
  if (!pifUart_AllocTxBuffer(&g_uart_log, 64)) return -1;											// 64bytes
  g_uart_log.act_start_transfer = actLogStartTransfer;

  pifLog_Init();
  if (!pifLog_AttachUart(&g_uart_log, 256)) return -1;												// 256bytes

  if (!pifTaskManager_Add(PIF_ID_AUTO, TM_PERIOD, 100000, _taskLedToggle, NULL, TRUE)) return -1;	// 100ms

  if (!appSetup()) return -1;

	pifLog_Print(LT_NONE, "\n\n****************************************\n");
	pifLog_Print(LT_NONE, "***           ex_pulse           ***\n");
